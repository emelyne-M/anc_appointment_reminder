...
def signup_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid() and patient_form.is_valid():
            ...
            # Create ANC appointments automatically
...
def dashboard_view(request):
    ...
    todays_visits = Appointment.objects.filter(
        scheduled_date=today
    ).count()

    missed_visits = Appointment.objects.filter(
...
def send_sms_reminders():
    ...
    for appointment in appointments:
        ...
        message = f"Hello {patient.full_name}, reminder for ANC visit {appointment.visit_number} tomorrow."
        ...
        if twilio_number:
            ...
            print(f"[TEST SMS] {patient.phone}: {message}")
